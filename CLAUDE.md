...
**Current version:** **1.1.0** — presets and custom fan modes. 250 unit tests passing across 14 suites. CI on Node 22 / 24 / 26. Tag-driven release pipeline publishing via npm trusted publishing (OIDC, no token). `ACCESSORY_SCHEMA_VERSION = 6`.
...
- **Some firmware advertises presets it does not implement.** A live SLWF-01Pro on ESPHome 2024.4.2 lists `[NONE, BOOST, ECO, SLEEP]` plus a custom `freeze protection`, accepts `preset`/`customPreset` commands without error, and stays on preset `0`. The same firmware answers `customFanMode: "silent"` by reporting plain `fanMode: 3` (LOW) with `customFanMode` empty. `mode` commands work normally on that device, so it is not a transport problem. This is [esphome/issues#3880](https://github.com/esphome/issues/issues/3880), open since 2022 — a bug in ESPHome's `midea` component, **not** stale firmware, so do not advise users to update to fix it. (1.1.1 shipped exactly that advice; corrected in 1.1.2.) `warnIfPresetIgnored()` exists because of this, and `disablePresets` defaults to `true`. Do not treat a capability list as proof of a capability.
- **Do not recommend updating SLWF-01Pro firmware.** [esphome/issues#7191](https://github.com/esphome/issues/issues/7191) — open since July 2025, multiple users on this exact dongle — reports soft/hard watchdog resets on every ESPHome version tested from 2024.6.6 through 2026.5.1, one user seeing a reboot every 20-30 minutes; another reports the preinstalled 2024.x worked fine until they updated. SMLIGHT's CI runs `pip install esphome` unpinned, so their builds ship whatever ESPHome was current that day. The test bench deliberately stays on 2024.4.2.
