...
function notifyCharacteristic(characteristic, value, forceEvent = false) {
	if (!characteristic) return;
	if (forceEvent && typeof characteristic.sendEventNotification === 'function') {
...
class DeviceAccessory {
	constructor({ device, deviceInfo, entities, platform }) {
		...
		// HAP-NodeJS updateValue only notifies when the value changes, so force
		// an event for the same-value "still healthy" startup clear.
		setTimeout(() => {
			if (this.connected !== false && this.HeaterCoolerService && Characteristic) {
				try {
					notifyCharacteristic(this.HeaterCoolerService.getCharacteristic(Characteristic.StatusActive), true, true);
	...
	setConnectedStatus(connected) {
		...
		if (connected) {
			...
			if (this._reconnectClearTimer) {
				clearTimeout(this._reconnectClearTimer);
				this._reconnectClearTimer = null;
			}
			try {
				this.HeaterCoolerService.getCharacteristic(Characteristic.StatusActive).updateValue(true);
				this.HeaterCoolerService.getCharacteristic(Characteristic.StatusFault)
			...
			// Delayed sendEventNotification for HomePod / iPhone that re-subscribes shortly
			// after reconnect. The immediate updateValue above covers currently-subscribed
			// controllers; this catches ones that re-establish their HAP session within the
			// next few seconds and would otherwise still see the stale GENERAL_FAULT.
			this._reconnectClearTimer = setTimeout(() => {
				this._reconnectClearTimer = null;
				if (this.connected !== false && this.HeaterCoolerService && Characteristic) {
					try {
						notifyCharacteristic(this.HeaterCoolerService.getCharacteristic(Characteristic.StatusActive), true, true);
					...
					} catch (_e) { /* reconnect delayed clear */ }
				}
			}, STARTUP_FAULT_CLEAR_DELAY_MS).unref();
		} else {
			// Cancel a pending reconnect-clear so a rapid disconnect-reconnect-disconnect
			// cycle doesn't push a false "healthy" event after the final disconnect.
			if (this._reconnectClearTimer) {
				clearTimeout(this._reconnectClearTimer);
				this._reconnectClearTimer = null;
			}
